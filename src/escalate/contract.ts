...
export function escalationContract(modelId: string, proModelId: string): string {
  ...
  return [
    ...
    `design trade-off you'd be guessing at — output this marker as the ENTIRE response — no`,
    `greeting, no "let me show you", no prose explanation, not even a leading blank line. The marker`,
    `IS the response; any text before it makes detection fail. This aborts the current call and`,
