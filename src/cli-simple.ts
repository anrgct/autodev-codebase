...
function formatSearchResults(results: SearchResult[], query: string): string {
  if (!results || results.length === 0) {
    return `[CLI] No results found for query: "${query}"`;
  }
...
${codeChunk}`;
  ...
  const summary = `[CLI] Found ${results.length} result${results.length > 1 ? 's' : ''} in ${fileCount} file${fileCount > 1 ? 's' : ''} for: "${query}"
...
function resolveOptions(): SimpleCliOptions {
  ...
  return {
    ...
    logLevel: (values['log-level'] as SimpleCliOptions['logLevel']) || 'info',
    demo: !!values.demo,
    force: !!values.force,
...
async function initializeManager(
  ...
  if (options.demo) {
    ...
    if (!workspaceExists) {
      ...
      console.log(`[CLI] Demo files created in: ${options.path}`);
    }
  }
  ...
  if (!validation.isValid) {
    console.warn('[CLI] Configuration validation warnings:', validation.errors);
  } else {
    console.log('[CLI] Configuration validation passed');
  }

  // Create CodeIndexManager
  console.log('[CLI] Creating CodeIndexManager...');
  const manager = CodeIndexManager.getInstance(deps);

  if (!manager) {
    console.error('[CLI] Failed to create CodeIndexManager - workspace root path may be invalid');
    return undefined;
  }

  // Initialize manager
  console.log('[CLI] Initializing CodeIndexManager...');
  await manager.initialize({ force: options.force, ...initOptions });
  console.log('[CLI] CodeIndexManager initialization success');
...
async function startMCPServer(options: SimpleCliOptions): Promise<void> {
  console.log('[CLI] Starting MCP Server Mode');
  console.log(`[CLI] Workspace: ${options.path}`);
  ...
  await server.start();
  console.log('[CLI] MCP Server started successfully');

  // Display configuration instructions
  console.log('\n[CLI] MCP Server is now running!');
  ...
  console.log('[CLI] Starting indexing process...');
  manager.onProgressUpdate((progressInfo) => {
    console.log(`[CLI] Indexing progress: ${progressInfo.systemStatus} - ${progressInfo.message || ''}`);
  });
  ...
  if (manager.isFeatureEnabled && manager.isInitialized) {
    manager.startIndexing()
      ...
      .catch((err: Error) => {
        console.error('[CLI] Indexing failed:', err.message);
      });
  } else {
    console.warn('[CLI] Skipping indexing - feature not enabled or not initialized');
  }

  // Handle graceful shutdown
  const handleShutdown = async () => {
    console.log('\n[CLI] Shutting down MCP Server...');
    await server.stop();
    console.log('[CLI] MCP Server stopped');
    process.exit(0);
  };
  ...
  console.log('[CLI] MCP Server is ready for connections. Press Ctrl+C to stop.');
...
async function indexCodebase(options: SimpleCliOptions): Promise<void> {
  console.log('[CLI] Starting indexing mode');
  console.log(`[CLI] Workspace: ${options.path}`);
  ...
  if (!manager.isFeatureEnabled) {
    console.error('[CLI] Code indexing feature is not enabled');
    process.exit(1);
  }
  ...
  return new Promise((resolve, reject) => {
    const checkState = () => {
      ...
      console.log(`[CLI] Current state: ${currentState}`);

      if (currentState === 'Indexed') {
        console.log('[CLI] Indexing completed successfully');
        resolve();
      } else if (currentState === 'Error') {
        console.error('[CLI] Indexing failed');
        reject(new Error('Indexing failed'));
      } else if (currentState === 'Standby') {
        console.warn('[CLI] Indexing stopped unexpectedly');
...
async function searchIndex(query: string, options: SimpleCliOptions): Promise<void> {
  ...
  console.log('[CLI] Searching index...');
  const results = await manager.searchIndex(query);

  if (results.length === 0) {
    console.log('[CLI] No results found');
    manager.dispose();
    return;
  }
  ...
  // 停止后台服务以允许程序退出
  manager.dispose();
  console.log('[CLI] Search completed. Exiting...');
}
...
async function clearIndex(options: SimpleCliOptions): Promise<void> {
  console.log('[CLI] Clear index mode');
  console.log(`[CLI] Workspace: ${options.path}`);
  ...
  console.log('[CLI] Clearing index data...');
  await manager.clearIndexData();
  console.log('[CLI] Index data cleared successfully');
}
