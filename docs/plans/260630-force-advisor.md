...
四条触发规则（预设 question 写死，仅 `mode === 'advisor'` 生效）：

| 规则 | 触发条件 | 预设 question |
|------|---------|--------------|
| user-turn | messages 末尾是 user 且 content **不含** `tool_result` | 「用户指令的核心需求？是否有歧义？」 |
| tool-error | messages 末尾是 user 含 `tool_result`，且末尾 `tool_result` 为 error | 「工具报错的原因？修复建议？」 |
| tool-count | messages 末尾是 user 含 `tool_result`，且 history 中真实 tool_use 总数 N>0 且 N%5==0 | 「当前方向是否正确？是否有错漏？下一步建议？」 |
| task-done | flash 响应内容不含 `tool_use`（proxy 在 buffer 内判定，非 messages 预处理） | 「任务完成了吗？」 |

**优先级/互斥规则：** 
- `tool-error` > `tool-count`（同属末尾 user含tool_result 类）
- `user-turn` 与 `tool-*` 互斥（末尾状态不同）
- `task-done` 与其他规则**独立**（在 flash 响应后判定，而非 messages 预处理阶段），但每个请求至多触发一次（`taskDoneTriggered` 标志防护）。
...
### 决策 6：配置 — 总开关默认关，question 写死，规则可独立开关

- `EscalateConfig.forceAdvisor`: `boolean | string`（默认 `false`）
  - `true` — 启用全部 4 条规则
  - `false` — 禁用
  - 逗号分隔的规则名列表，如 `"user-turn,tool-error"` — 仅开启指定规则
- 四个 question + tool-count 间隔（5）写死为模块常量，不进配置
- 仅 `mode === 'advisor'` 下生效
- 有效规则名：`user-turn`, `tool-error`, `tool-count`, `task-done`
...
### 2026-07-04 — 规则 4(task-done) + config 字符串规则名过滤

**问题（需求）：**
1. 需要第四条规则：当 flash 回答时没有 tool call（任务结束），触发 forced advisor 问「任务完成了吗？」。以前只有 user-turn / tool-error / tool-count 三条。
2. `escalateForceAdvisor` 只支持 `true/false`，无法只开启部分规则。需要支持逗号分隔的规则名列表，如 `"user-turn,task-done"`。

**修复：**
1. **规则 4（task-done）：** 实现在 post-response 阶段（非 messages 预处理）—— `dispatchAdvisorNonStream` flash 响应不含 tool_use 时触发，咨询 pro 后将结果拼接进 `workingMessages` 后 flash retry。流式同理：`peekAdvisorStream` 末尾无 tool_use 返回 `task-done` outcome，dispatch 循环将 text 重写为 thinking block → 咨询 pro → flash retry。每请求至多一次（`taskDoneTriggered` 标志）。
2. **config 字符串规则名过滤：**
   - `EscalateConfig.forceAdvisor` 类型从 `boolean` 改为 `boolean | string`。
   - 新增 `resolveForceAdvisorRules(forceAdvisor)`：`true`/`"true"` → `'all'`；`false`/`"false"` → `false`；`"rule1,rule2"` → `Set<ruleName>`（无效名忽略）。
   - 新增 `isTriggerEnabled(trigger, enabledRules)`：检查某触发器是否被当前 config 允许。
   - 非流式/流式 dispatch 中：先 `resolve` 再检测 `isTriggerEnabled`。
3. **配置贯通：**
   - `src/escalate/types.ts`：`forceAdvisor: boolean | string`
   - `src/commands/escalate.ts`：`loadEscalateConfig` 透传 string；banner 显示规则名
   - `src/code-index/interfaces/config.ts`：`escalateForceAdvisor?: boolean | string`
   - `src/commands/config/metadata.ts`：描述更新
4. **证据脚本：** `scripts/evidence/260630-forced-advisor.ts` 新增 `rule4-task-done` 测试用例；`PRESET_QUESTIONS` 加 `task-done`；header 更新为四条规则。

**新增导出：**
- `FORCE_ADVISOR_RULE_NAMES` — 有效规则名常量数组
- `ForcedAdvisorRuleName` — 规则名联合类型
- `resolveForceAdvisorRules` — config→rules 解析函数
- `isTriggerEnabled` — trigger×rules 判定函数

**改动文件：**

| 文件 | 改动 |
|------|------|
| `src/escalate/dispatcher.ts` | `detectForcedAdvisor` 加 task-done 判定；新增 `FORCE_ADVISOR_TASK_DONE_QUESTION`、`FORCE_ADVISOR_RULE_NAMES`、`resolveForceAdvisorRules`、`isTriggerEnabled`；非流式/流式 dispatch 加规则过滤 |
| `src/escalate/types.ts` | `forceAdvisor` 类型 `boolean` → `boolean | string` |
| `src/commands/escalate.ts` | 加载逻辑透传 string；banner 显示规则详情 |
| `src/code-index/interfaces/config.ts` | `escalateForceAdvisor?: boolean | string` |
| `src/commands/config/metadata.ts` | 描述更新 |
| `src/escalate/__tests__/dispatcher.spec.ts` | 新增 15 测试：task-done(2) + 不触发(1) + resolve 纯函数(8) + isTriggerEnabled(3) + task-done 集成(1) + 规则过滤集成(1) |
| `scripts/evidence/260630-forced-advisor.ts` | 新增 rule4-task-done 用例；header/PRESET_QUESTIONS 同步 |

**验证：**
- type-check 零错误
- escalate 全套 7 文件 / 190 测试全过（原 175 + 新增 15），零回归
...
| 日期 | 问题 | 修复 |
|------|------|------|
| 07-04 (fix 1) | flash 回复在 stream task-done 路径中未拼入 `workingMessages`，pro 看不到上下文 | 两处 splice 补 `flashAssistantMsg`；非流式同步确认已正确 |
| 07-04 (fix 2) | task-done 只在 `hasPendingTextBlock` 时触发，thinking-only 回复被漏掉 | `peekAdvisorStream` 条件改为 `hasPendingTextBlock \|\| pendingThinkingAcc.length > 0`；新增 `pendingThinkingAcc` 累计 thinking delta |
| 07-04 (fix 3) | pro prompt 中 "0 tool so DO NOT CALL" 与 `finish` tool 矛盾 | `FORCED_ADVISOR_PROMPT_PREFIX` 和 `ADVISOR_CONSULT_PROMPT_PREFIX` 移除"do not call any tools" |

**验证（最新）：** escalate 全套 7 文件 / 190 测试全过，零回归。
...
3. **stateless 去重** — 基于 messages 末尾状态判断（纯 user → 规则 1；含 tool_result → 规则 3/2；assistant 无 tool_use → 规则 4），无需内存计数器或 messages 内嵌 sentinel。
...
**使用方式：** 配置 `escalateForceAdvisor: true|false|规则名列表`（仅 `mode: 'advisor'` 生效，默认 false）。
- `true` — 全部 4 条规则
- `false` — 禁用
- `"user-turn,tool-error"` — 仅启用指定规则
