...
export function escalationContract(modelId: string, proModelId: string): string {
  if (modelId === proModelId) {
    ...
    return [
      ...
      `read-only file inspections where the answer is obvious — output this marker as the FIRST line`,
      `of your response (nothing before it, not even whitespace on a separate line). This aborts the`,
      `current call and retries this turn on the cheaper flash model, saving cost and latency.`,
      ``,
      `Two accepted forms:`,
  ...
  return [
    `${CONTRACT_SENTINEL} Cost-aware tier switching instruction (you are running on \`${modelId}\` — the cheap tier):`,
    ``,
    `Two markers are available across the system — \`<<<NEEDS_PRO>>>\` and \`<<<NEEDS_FLASH>>>\`.`,
    `Here is how they apply to YOU:`,
    ``,
    `### \`<<<NEEDS_PRO>>>\` — escalation request (ACTIVE for you)`,
    ...
    `design trade-off you'd be guessing at — output this marker as the FIRST line of your response`,
    ...
    ``,
    `### \`<<<NEEDS_FLASH>>>\` — downgrade request (NO-OP for you)`,
    `You are already the flash (cheap) tier. There is no lower tier to downgrade to.`,
    `If you see this marker in the conversation history or consider emitting it yourself,`,
    `ignore it — it does not apply to you. Simply proceed with your normal response.`,
    ``,
