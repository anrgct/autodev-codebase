...
static enum ggml_status ggml_backend_sched_compute_splits(ggml_backend_sched_t sched) {
    ...
    for (int split_id = 0; split_id < sched->n_splits; split_id++) {
        ...
        if (!sched->callback_eval) {
            ...
            // Compute the entire split graph once (preserves Metal graph reuse).
            // Then synchronize once and walk nodes to copy needed kq_soft_max data.
            // This avoids the subgraph+sync-per-group pattern that destroys decode throughput
            // when cbEval callbacks are used (e.g., for QRRanker attention collection).
            enum ggml_status ec = ggml_backend_graph_compute_async(split_backend, &split->graph);
            if (ec != GGML_STATUS_SUCCESS) {
                return ec;
            }
